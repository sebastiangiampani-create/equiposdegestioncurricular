const chapters = [
  {
    ...
    ideas: [
      ...
      "El liderazgo pedagogico no se define por la autoridad jerarquica, sino por la capacidad de abrir conversaciones y construir acuerdos para mejorar la ensenanza.",
      "Si lo pedagogico no se prioriza de forma explicita, la urgencia administrativa y la burocracia terminan devorando el tiempo y el sentido de la escuela.",
      "Revisitar la escuela implica mirar lo cotidiano con ojos nuevos para poner en duda aquellas rutinas que damos por sentadas pero que pueden transformarse.",
      "La gramatica escolar tradicional tiende a absorber las reformas; por eso es necesario cambiar la estructura de fondo y no solo aplicar cambios superficiales.",
      "La herramienta principal del asesoramiento es la formulacion de preguntas potentes que ayuden a los docentes a reflexionar sobre sus propias practicas.",
      "El equipo de gestion solo puede orientar a otros si primero construye un discurso comun y propio sobre el proyecto educativo que quiere llevar adelante."
    ]
  },
  {
    label: "Capitulo 2",
    ...
    ideas: [
      ...
      "Construir el PCI es un acto politico, ya que implica decidir colectivamente que saberes son irrenunciables y que tipo de estudiante queremos formar.",
      "La brecha entre la intencion y la realidad no es un fracaso, sino la informacion mas valiosa que tiene el equipo de gestion para intervenir y transformar la ensenanza.",
      "La especificacion es el camino necesario, pues permite adaptar la norma a las necesidades de la comunidad preservando el sentido comun de la politica educativa.",
      "Integrar saberes desafia la identidad docente, ya que requiere aflojar las fronteras rigidas de cada materia para construir un conocimiento mas complejo y compartido.",
      "Un laboratorio no es una suma de temas, sino un espacio donde las disciplinas funcionan como recursos para comprender y resolver problemas reales.",
      "La progresion por niveles rompe con la repitencia, al priorizar los aprendizajes efectivamente logrados por sobre el mero paso del tiempo cronologico."
    ]
  },
  {
    label: "Capitulo 3",
    ...
    ideas: [
      ...
      "Un problema genuino se distingue de un tema porque exige analisis, interpretacion y toma de posicion, superando las preguntas meramente descriptivas.",
      "Las preguntas poderosas son el motor de la indagacion: son abiertas, no tienen una respuesta unica y no se resuelven con una busqueda rapida.",
      "La integracion de saberes es un proceso gradual que escala desde la simple mencion entre materias hasta el trabajo conjunto sobre un eje compartido.",
      "En el nivel de globalizacion, el problema central manda sobre las disciplinas, obligandolas a transformar sus secuencias y metodos para abordar la complejidad del desafio.",
      "Lograr una integracion autentica requiere confianza entre colegas y disposicion para flexibilizar el control sobre el programa individual en favor de una planificacion coordinada."
    ]
  },
  {
    label: "Capitulo 4",
    ...
    ideas: [
      ...
      "La repitencia es un sintoma de un sistema rigido: el desajuste entre trayectorias teoricas y trayectorias reales produce fracaso escolar.",
      "El avance continuo propone que la progresion escolar se defina por las capacidades y aprendizajes logrados, no solo por el transcurso de un ano.",
      "Organizar la escuela por niveles pedagogicos exige que los equipos docentes definan en el PCI que debe saber hacer un estudiante para pasar de un nivel al siguiente.",
      "La autonomia estudiantil es un contenido a ensenar: la escuela debe andamiar y acompanar la capacidad de elegir y tomar decisiones.",
      "La electividad permite que estudiantes con diferentes intereses lleguen a aprendizajes irrenunciables a traves de propuestas distintas pero equivalentes."
    ]
  },
  {
    label: "Capitulo 5",
    ...
    ideas: [
      ...
      "Un diseno pedagogico solido se construye bajo la logica del diseno inverso: los objetivos de aprendizaje deben guiar cada actividad propuesta.",
      "El punto de partida no es un tramite formal, sino una instancia diagnostica clave para obtener informacion real sobre los saberes previos.",
      "La calidad de la indagacion se define por quien realiza el trabajo cognitivo: el estudiante debe ocupar un rol activo de analisis y argumentacion.",
      "La produccion final debe funcionar como evidencia de comprension, demostrando uso autonomo de los conocimientos en situaciones nuevas.",
      "La coherencia interna entre lo que se espera aprender, lo que se hace y lo que se evalua transforma actividades sueltas en un sistema de ensenanza.",
      "El rol directivo consiste en brindar asesoramiento situado, usando el plan como ventana para visibilizar decisiones pedagogicas y mejorar la practica."
    ]
  },
  {
    label: "Capitulo 6",
    ...
    ideas: [
      ...
      "El diseno inverso propone definir primero los resultados deseados y las evidencias de aprendizaje antes de elegir actividades o materiales.",
      "La potencia de la ensenanza reside en la tarea cognitiva: lo que el estudiante aprende depende de la demanda que plantea la consigna.",
      "Las consignas deben ser trampolines y no vallas: plataformas que impulsan produccion propia y autonomia, no obstaculos para una respuesta unica.",
      "Desarrollar capacidades exige ampliar el repertorio docente, integrando estrategias como la clase socratica o el aprendizaje basado en problemas.",
      "Diversificar la ensenanza es un compromiso con la equidad: ofrecer multiples caminos, procesos y soportes para aprendizajes de alta calidad."
    ]
  }
];
...
function buildChapterButtons() {
  chapters.forEach((chapter, index) => {
    ...
    button.className = index === 0 ? "chapter-btn active" : "chapter-btn";
    button.type = "button";
    button.dataset.index = String(index);
    button.innerHTML = `<span>${chapter.label}</span>${chapter.title}`;
    button.addEventListener("click", () => renderChapter(index, true));
...
function renderChapter(index, autoplay = false) {
  ...
  document.querySelectorAll(".chapter-btn").forEach((button, buttonIndex) => {
  ...
  if (autoplay) {
    ...
    if (playPromise !== undefined) {
      playPromise.catch(() => {
        // Algunos navegadores bloquean autoplay. El usuario puede tocar play manualmente.
      });
    }
  }
}
